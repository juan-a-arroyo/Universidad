...
class ChatBotGUI:
    def __init__(self, master):
        ...
        self.chat_log = tk.Text(self.chat_frame, wrap=tk.WORD)
        self.chat_log.pack(side=tk.LEFT, expand=True, fill=tk.BOTH)
    ...
    def greet_user(self):
        self.display_message("¡Hola! Soy un ChatBot. ¡Pregunta lo que quieras!")
    ...
    def respond_to_question(self, question):
        pregunta_limpiada = self.limpiar_pregunta(question)
        
        if pregunta_limpiada in self.respuestas:
            return random.choice(self.respuestas[pregunta_limpiada])
