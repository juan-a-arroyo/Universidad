...
def main():
    pygame.init()
    screen = pygame.display.set_mode((WINDOW_SIZE, WINDOW_SIZE + 50))  # Aumentamos la altura de la ventana
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
            elif event.type == pygame.MOUSEBUTTONDOWN and env.game_over:
                env = Environment()
                agent.visited.clear()
        ...
        if env.all_clean():
            # Mostrar "WIN!" junto al contador de movimientos
            win_text = font.render("WIN!", True, YELLOW)
            screen.blit(win_text, (10, WINDOW_SIZE + 10))

        # Mostrar el número de movimientos
        move_text = font.render("Moves: {}".format(env.moves), True, BLACK)
        screen.blit(move_text, (150, WINDOW_SIZE + 10))
