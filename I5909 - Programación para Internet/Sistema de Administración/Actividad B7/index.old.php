<!doctype html>
<html>
    <head>
        <meta charset="UTF-8">
        <title>Login</title>
    </head>
    <link href="Administrador/css/style.css" type="text/css" rel="stylesheet">
    <script src="Administrador/js/jquery-3.3.1.min.js"></script>
    <script>
        function autentificar() {
            var usuarioInput = $('#usuario');
            var passInput = $('#pass');
            var usuario = usuarioInput.val();
            var pass = passInput.val();
            $.ajax({
                url: 'Administrador/Funciones/validar_correo.php',
                method: 'POST',
                data: { correo: usuario },
                success: function(res) {
                    if (res == 1){
                        $.ajax({
                            url: 'Administrador/Funciones/validar_pass.php',
                            method: 'POST',
                            data: { correo : usuario, pass : pass },
                            success: function(res_) {
                                if (res_ == 1){
                                    window.location.href = 'bienvenido.php';
                                }
                                else {
                                    if(pass == "" && usuario != "")
                                        $('#user-error').text('Introduzca una contraseña').show();
                                    else
                                        $('#user-error').text('Usuario o contraseña inválidos').show();
                                    setTimeout(function() {
                                        $('#user-error').hide();
                                    }, 5000);
                                }
                            }
                        });
                    }
                    else{
                        if(pass == "" && usuario != "")
                            $('#user-error').text('Introduzca una contraseña').show();
                        else
                            $('#user-error').text('Usuario o contraseña inválidos').show();
                        setTimeout(function() {
                            $('#user-error').hide();
                        }, 5000);
                    }
                }
            });
        }
    </script>
    <body>
        <div>
            <table>
                <tr>
                    <td class="title">LOGIN</td>
                </tr>
                <tr>
                    <td class="first">Usuario:
                        <input type="text" name="usuario" id="usuario" placeholder="admin@mail.com" required>
                    </td>
                </tr>
                <tr>
                    <td class="first">Contraseña:
                        <input type="password" name="pass" id="pass" placeholder="Contraseña" required>
                    </td>
                </tr>
                <tr>
                    <td class="bottom">
                        <input class="button" id="darkGray" type="submit" value="Iniciar Sesión" onclick="return autentificar();">
                        <div id="user-error" class="message"></div>
                    </td>
                </tr>
            </table>
        </div>
    </body>
</html>