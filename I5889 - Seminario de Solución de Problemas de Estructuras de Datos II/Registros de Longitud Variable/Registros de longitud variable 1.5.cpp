...
class Lista{
    ...
    public:
        ...
        int contar_elementos(const Lista &lista){
        ...
        friend ostream& operator << (ostream &out, const Lista &lista){
            ...
            int posicion = 1;
            while(producto != nullptr){
                out<<endl<<"|"<<posicion<<"|"<<"Nombre: "<<producto->getNombre()<<"|"<<
                "Costo: "<<producto->getCosto()<<"|"<<
                "Precio: "<<producto->getPrecio()<<"|"<<
                "Codigo: "<<producto->getCodigo()<<"|"<<
                "Existencias: "<<producto->getExixtencias()<<"|";
                posicion++;
                producto = producto -> next;
            }
            out<<endl<<endl;
...
int main (void){
    ...
    do{
        ...
        switch(opcion){
            ...
            case 7:
                if(!guardado){
                    cout<<"Hay cambios no guardados"<<endl<<endl<<"1. Guardar y Salir"<<endl<<"2. Salir sin guardar"<<endl<<endl<<"Opcion: ";
                    guardar = validar_rango(1,2);
                    if(guardar = 1)
                        productos.escribir(nombreArchivo);
                }
            break;
...
void leer(string nombreArchivo, Lista productos){
    ifstream archivo;
    string texto, nombre, codigo;
    float costo, precio;
    int existencias, posicion;
        ...
        while(getline(stream, registro, ';')){
            ...
            while(getline(stream, campo, ',')){
                switch(posicion){
                    ...
                    break;
                    case 4:
                        existencias = stoi(campo, nullptr, 10);
                    break;
                }    
            }
            productos.agregar(nombre, costo, precio, codigo, existencias);
...
int validar_rango(int min, int max){
    int val;
    cin>>val;
    while(!(val >= min && val <= max)){
        cout<<"Digite un numero valido ("<<min<<"-"<<max<<"):";
        cin>>val;
    }
    return val;
}
